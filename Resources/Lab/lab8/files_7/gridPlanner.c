// Main program to plan an electrical grid
...
static void readPlaces(struct place cities[], int *numCities,
                       struct place powerPlants[], int *numPowerPlants);
static bool readPlace(struct place *p, bool *isPowerPlant);
static void chomp(char *s);

static void outputPlain(struct place cities[], int numCities,
                        struct place powerPlants[], int numPowerPlants,
                        struct powerLine powerLines[], int numPowerLines);
static void outputVisualiser(struct place cities[], int numCities,
                             struct place powerPlants[], int numPowerPlants,
                             struct powerLine powerLines[], int numPowerLines);
...
int main(int argc, char *argv[]) {
    ...
    struct place cities[MAX_CITIES];
    int numCities = 0;

    struct place powerPlants[MAX_POWER_PLANTS];
    int numPowerPlants = 0;

    readPlaces(cities, &numCities, powerPlants, &numPowerPlants);

    int numPlaces = numCities + numPowerPlants;
    struct powerLine *powerLines = calloc(numPlaces * numPlaces,
                                          sizeof(struct powerLine));
...
static void outputPlain(struct place cities[], int numCities,
    ...
    for (int i = 0; i < numPowerLines; i++) {
        struct powerLine *l = &(powerLines[i]);
